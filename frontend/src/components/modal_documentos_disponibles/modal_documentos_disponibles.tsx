      ...
      <div className="mdd-panel" onClick={(e) => e.stopPropagation()}>
        <header className="mdd-header">
          <div>
            ...
            <p className="mdd-subtitle">Explora los {fuentes.length} archivos de esta colección</p>
          </div>
              ...
              fuentesFiltradas.map((f, i) => (
                ...
                >
                  <div className="mdd-card-icon">
                    {f.estado === 'error' ? <AlertCircle size={20} color="#ef4444" /> : <FileText size={20} />}
                  </div>
                  <div className="mdd-card-content">
                    <span className="mdd-card-title">{f.titulo || f.name || f.nombre}</span>
                    <div className="mdd-card-meta">
                      <span className="mdd-tag">{f.tipo || f.extension || 'DOC'}</span>
                      {f.estado === 'error' && <span className="mdd-error-tag">Error de carga</span>}
                    </div>
                  </div>
