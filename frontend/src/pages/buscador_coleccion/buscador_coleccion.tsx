...
const CORPUS: Resultado[] = [
  ...
  { id: 5, fuenteId: 3, fuenteTitulo: "Dataset H&M Chile - Outfits",         fuenteTipo: "CSV", extracto: "Columnas: article_id, product_type_name, colour_group_name, perceived_colour_value. Total filas: 105.542." },
...
function Highlight({ text, query }: { text: string; query: string }) {
  ...
  return <>{parts.map((part, i) => regex.test(part) ? <mark key={i} className="bc-hl">{part}</mark> : part)}</>;
}
...
const BuscadorColeccion = () => {
  ...
  const resultados = CORPUS.filter(r => {
    ...
    const matchTexto = r.extracto.toLowerCase().includes(busqueda.toLowerCase()) ||
                       r.fuenteTitulo.toLowerCase().includes(busqueda.toLowerCase());
        ...
        <main className="bc-main">
          ...
          <div className="bc-searchbar-wrap">
            <div className="bc-searchbar">
              ...
              <input className="bc-searchbar-input" type="text" placeholder="Busca en tus fuentes..." value={busqueda} onChange={e => setBusqueda(e.target.value)} autoFocus />
              {busqueda && (
                <button className="bc-searchbar-clear" onClick={() => setBusqueda("")}><X size={14} /></button>
              )}
              <div className="bc-searchbar-divider" />
              <button className={`bc-filter-btn${filtroOpen ? " active" : ""}`} onClick={() => setFiltroOpen(!filtroOpen)}>
            ...
            {filtroOpen && (
              <div className="bc-filter-panel">
                ...
                <div className="bc-filter-chips">
                  {TIPOS.map(t => (
                    <button key={t} className={`bc-filter-chip${filtroTipo === t ? " selected" : ""}`} onClick={() => setFiltroTipo(t)}>
                      {t === "todos" ? "Todos" : t}
                    </button>
                  ))}
                </div>
