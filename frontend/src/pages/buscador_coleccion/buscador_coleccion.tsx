...
const BuscadorColeccion = () => {
  ...
  const confirmarBorrado = () => {
    ...
    const path = id_usuario ? `/${id_usuario}` : '/'
    navigate(path)
  }
  ...
  const agregarTag = (val: string, lista: string[], setLista: (v: string[]) => void, setInput: (v: string) => void) => {
    ...
    if (trimmed && !lista.includes(trimmed)) {
      ...
      <div className={`bc-root${darkMode ? ' bc-dark' : ''}`}>
        <aside className="bc-sidebar">
          <div className="bc-sidebar-inner">
            ...
            <button className="bc-add-btn" onClick={() => setModalGrafoOpen(true)}><Network size={15} /> <span>Ver Grafo</span></button>
            <button className="bc-add-btn" onClick={() => setIsModalFuentesOpen(true)}><Files size={15} /> <span>Ver Documentos</span></button>
            <button className="bc-delete-collection-btn" onClick={() => setIsEliminarModalOpen(true)}><Trash2 size={14} /> <span>Borrar colección</span></button>
          </div>
        </aside>
          ...
          <div className="bc-results-area">
            {resultados.length > 0 ? (
              ...
              <div className="bc-empty">
                ...
                <p className="bc-empty-title">{busquedaEnviada.trim() ? 'No hay resultados' : 'Busca en tu colección'}</p>
              </div>
            )}
          </div>
        </main>
