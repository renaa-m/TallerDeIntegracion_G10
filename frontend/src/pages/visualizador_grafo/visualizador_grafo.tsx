...
const GraphViewer = () => {
  ...
  const runLayout = useCallback((cyInstance: cytoscape.Core | null) => {
  ...
  const fetchGraph = useCallback(async () => {
    ...
    try {
      ...
      const edges: cytoscape.ElementDefinition[] = (data.elements?.edges ?? []).map((e, index) => {
        const rawData = e.data || {}
        const sourceId = rawData.source || rawData.source_id || rawData.id_origen || rawData.from
        const targetId = rawData.target || rawData.target_id || rawData.id_destino || rawData.to
  ...
  return (
    <div ref={containerRef} className="gv-outer-wrapper">
      ...
      {elements.length === 0 && !errorMsg && (
        <div className="gv-overlay-card"><div className="gv-card-info"><p>Colección vacía.</p></div></div>
      )}
      {errorMsg && (
        <div className="gv-overlay-card"><div className="gv-card-info gv-card-error"><p>{errorMsg}</p></div></div>
      )}
          ...
          <div className="gv-inspector-header">
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span className="gv-inspector-title">
                {selectedType === 'node' ? 'Detalle Entidad' : 'Detalle Relación'}
              </span>
            </div>
            <button className="gv-inspector-close" onClick={() => { cyRef?.elements().unselect(); setSelectedData(null); setSelectedType(null); }}>
              <X size={15} />
            </button>
          </div>
          <div className="gv-inspector-body">
            <div className="gv-badge-container">
              <span className="gv-type-badge">{selectedType === 'node' ? `Tipo: ${selectedData.tipo || 'General'}` : 'Predicado'}</span>
              <p className="gv-main-label-display">{selectedData.label || 'Sin etiqueta'}</p>
            </div>
