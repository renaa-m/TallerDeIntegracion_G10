...
let mockParams = {
    ...
    jest.mock('../components/modal_eliminar_coleccion/modal_eliminar_coleccion', () => {
    ...
    jest.mock('../components/modal_documentos_disponibles/modal_documentos_disponibles', () => {
    ...
    test('muestra estado vacío inicial cuando no hay búsqueda enviada', async () => {
        ...
        expect(
        screen.getByText('Haz una consulta para explorar los documentos de esta colección.'),
        ).toBeInTheDocument()
    })
    ...
    test('ejecuta búsqueda al presionar Enter y muestra resultados', async () => {
        ...
        fireEvent.change(screen.getByPlaceholderText('Consulta algo a tus documentos...'), {
        target: { value: 'grafos' },
        })

        fireEvent.keyDown(screen.getByPlaceholderText('Consulta algo a tus documentos...'), {
        key: 'Enter',
        })
        ...
        expect(await screen.findByText('Documento sobre grafos')).toBeInTheDocument()
    ...
    test('si búsqueda retorna 422 muestra mensaje sin resultados', async () => {
        ...
        expect(
        await screen.findByText('No hay fragmentos que coincidan con tu búsqueda semántica.'),
        ).toBeInTheDocument()
    })
    ...
    test('permite abrir y cerrar banner de filtros', async () => {
        ...
        expect(screen.getByText('Próximamente: Filtros Avanzados')).toBeInTheDocument()

        const closeButton = document.querySelector('.bc-alert-close') as HTMLButtonElement
        fireEvent.click(closeButton)

        expect(screen.queryByText('Próximamente: Filtros Avanzados')).not.toBeInTheDocument()
    })
    ...
    test('abre y cierra modal de eliminar colección sin borrar', async () => {
        ...
        fireEvent.click(screen.getByRole('button', { name: /cancelar eliminación/i }))

        expect(screen.queryByTestId('modal-eliminar-coleccion')).not.toBeInTheDocument()
    })
